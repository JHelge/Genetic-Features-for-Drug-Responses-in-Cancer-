...
def converter(i) :
	...
	dfData = read_csv(dataDirectory+"/X_data.csv", header=None, sep=',', dtype='unicode', low_memory=False)
	...
	#plt.hist(dfLabels, bins=int(dfLabels.size/20), color='skyblue', edgecolor='black') # 'bins' legt die Anzahl der Bins fest
	#plt.xlabel('Werte')
...
def main():
	threads=-1
	totalRuns=2500
	Parallel(n_jobs=threads, verbose=5, backend="multiprocessing")(delayed(converter)(i) for i in range(1003,totalRuns))
	return
main()
#converter(0)
def createDataFile() :
